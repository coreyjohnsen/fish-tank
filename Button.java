//////////////// Fish Tank 3000 //////////////////////////
//
// Title: Button.java
// Course: CS 300 Fall 2021
//
// Author: Corey Johnsen
// Email: cjjohnsen@wisc.edu
// Lecturer: Hobbes LeGault
//
///////////////////////// ALWAYS CREDIT OUTSIDE HELP //////////////////////////
//
// Persons: None
// Online Sources: None
//
///////////////////////////////////////////////////////////////////////////////

/**
 * An instantiable class that represents a button in the tank
 */
public class Button implements TankListener {

  private static final int WIDTH = 85; // Width of this Button
  private static final int HEIGHT = 32; // Height of this Button
  protected static FishTank tank; // PApplet object where this button will be displayed
  private float x; // x-position of this button in the display window
  private float y; // y-position of this button in the display window
  protected String label; // text/label which represents this button

  /**
   * Creates a new AddBlueFishButton
   * 
   * @param label is the text on the button
   * @param x     is the x position of the button
   * @param y     is the y position of the button
   */
  public Button(String label, float x, float y) {
    this.label = label;
    this.x = x;
    this.y = y;
  }

  /**
   * Sets the processing of the Button
   * 
   * @param tank is the FishTank to set the processing to
   */
  public static void setProcessing(FishTank tank) {
    Button.tank = tank;
  }

  /**
   * Draws the button to the window
   */
  @Override
  public void draw() {
    // sets the color of the button based on whether the mouse is over it
    if (isMouseOver()) {
      tank.fill(100);
    } else {
      tank.fill(200);
    }

    // draw the button (rectangle with a centered text)
    tank.rect(x - WIDTH / 2.0f, y - HEIGHT / 2.0f, x + WIDTH / 2.0f, y + HEIGHT / 2.0f);
    tank.fill(0);
    tank.text(label, x, y); // display the text of the current button
  }

  /**
   * Prints statement when the button is clicked
   */
  @Override
  public void mousePressed() {
    if (isMouseOver()) {
      System.out.println("A button was pressed.");
    }
  }

  /**
   * Defines behavior of button when mouse is released
   */
  @Override
  public void mouseReleased() {
    // empty
  }

  /**
   * Determines if the mouse is over the button
   * 
   * @return true if mouse is over button, false otherwise
   */
  @Override
  public boolean isMouseOver() {
    // checks if the mouse is over this object
    return tank.mouseX >= x - WIDTH / 2 && tank.mouseX <= x + WIDTH / 2
        && tank.mouseY >= y - HEIGHT / 2 && tank.mouseY <= y + HEIGHT / 2;
  }
}
